...
public class PlayerControl : MonoBehaviour
{
    ...
    public GameObject BurnerController;
    ...
    public GameObject HealingPlace;
    public GameObject FlameThrowingEffect;
    ...
    public AudioClip RandomThunderClip;
    public AudioClip CastLightningClip;
    public AudioClip HealingPlaceCreatedClip;
    public AudioClip BearBasicAttackClip;
    ...
    public float IceBallShootForce;
    ...
    public float FlamingCoolDown;
    public float PreFlamingAnimWait;
    public float AftFlamingAnimWait;
    ...
    public float TotemIncreasedAA;
    public float TotemIncreasedTime;
    ...
    float GetMAXCD(int id) {
        switch(id) {
            ...
            case 201: return FlamingCoolDown;
    ...
    private int LightningAttackCnt;
    void Attack() {
        if (Doing || HitBack)
            return;
        if (!BearMode) {
            if (Input.GetKey(KeyCode.A) && !Firing && !LightningCast) {
                ...
                ToggleNavi();
                LightningAttackCnt++;
                if (LightningAttackCnt == 3) {
                    StartCoroutine(ChainLightning(Ene));
                    LightningAttackCnt = 0;
                }
                else
                    StartCoroutine(LightningAA(Ene, 0));
            ...
            if (Input.GetKey(KeyCode.A) && !Firing) {
                ...
                if (BearAttackCnt == 0) {
                    ...
                    audioPlayer.PlayOneShot(BearThirdAttackClip);
        ...
        foreach (var obj in hit) {
            if (obj.transform.root.CompareTag("Enemy") || obj.transform.root.CompareTag("Monster")) {
                ...
                if (!enemies.ContainsKey(hash)) {
                    DataManager.Instance.takedamage(ChangeToEnemyTrans(obj.transform.root), damage);
    ...
    IEnumerator LightningAA(GameObject Enemy, int x) {
        PlayerAnim.SetInteger("Doing", 8);
        Invoke("ResetAnimDoing", 0.2f);
        audioPlayer.PlayOneShot(ZapClip);
        ...
        StartCoroutine(LightningInstantiate(Enemy, x));
    ...
    IEnumerator LightningInstantiate(GameObject Target, int x) {
        ...
        GameObject LightningPrefab = null;
        if (x == 0) {
            LightningPrefab = Instantiate(Lightning, rightHand.position, Quaternion.identity);
        } else {
            LightningPrefab = Instantiate(Lightning2, rightHand.position, Quaternion.identity);
        }
    ...
    void Frost(Vector3 SubTarget) {
        ...
        Doing = true;
        Target.y = 0;
        StartCoroutine(ShootIceBall(Target));
    ...
    IEnumerator ShootIceBall(Vector3 Target) {
        yield return new WaitForSeconds(FrostWaitTime);
        GameObject IceBallPrefab = Instantiate(IceBall, (rightHand.position + leftHand.position) / 2.0f, Quaternion.LookRotation(Target));
        Rigidbody IceBallRb = IceBallPrefab.GetComponent<Rigidbody>();
        IceBallRb.AddForce(IceBallShootForce * Target.normalized);
    }
    ...
    IEnumerator BecomeHumanAnimation() {
        ...
        CancelInvoke("ResetAnimDoing");
        DeactiveNavi();
        Vector3 TurnInto = new Vector3(HumanScale*Scaling*BearHumanScaleRatio, HumanScale*Scaling*BearHumanScaleRatio, HumanScale*Scaling*BearHumanScaleRatio);
    ...
    private bool FlamingCD;
    private float CurFlamingCD;
    void Flaming() {
        Doing = true;
        FlamingCD = true;
        AvoidCasting = true;
        ...
        FacingTarget = Pos - Bear.transform.position;
        Instantiate(BurnerController, Bear.transform.position, Quaternion.identity);
        BurnerManager.Instance.MaxBurnTime = MaxBurnTime;
        BurnerManager.Instance.BurnFreq = BurnFreq;
        BurnerManager.Instance.BurnDamage = BurnDamage;
        BearAnim.SetTrigger("Buff");
        StartCoroutine(CastFlaming(FacingTarget));
        StartCoroutine(CoolDownCal(FlamingCoolDown, (returnVal1, returnVal2) => {
            CurFlamingCD = returnVal1;
            FlamingCD = returnVal2;
        }));
    }

    IEnumerator CastFlaming(Vector3 Target) {
        yield return new WaitForSeconds(PreFlamingAnimWait);
        Instantiate(FlameThrowingEffect, BearMouth.position, Quaternion.LookRotation(Target));
        yield return new WaitForSeconds(AftFlamingAnimWait);
        Doing = false;
        AvoidCasting = false;
        BearOnCast = false;
        ToggleNavi();
    ...
    IEnumerator KnockOnFloor() {
        ...
        audioPlayer.PlayOneShot(EarthQuakeClip);
    ...
    IEnumerator ShieldBreak() {
        ...
        audioPlayer.PlayOneShot(ShieldBreakClip);
    ...
    IEnumerator CastLightning(Vector3 Target) {
        PlayerAnim.SetInteger("Doing", 8);
        Invoke("ResetAnimDoing", 0.3f);
        audioPlayer.PlayOneShot(CastLightningClip);
    ...
    void ChargingTotem(float amount) {
        ...
        if (LightningCast) {
            ...
            if (TotemCharged >= 100f) {
                ...
                ChLightning = true;
                StartCoroutine(IncreaseLightningReload());
                TotemCharged = 0;
            }
        } else {
    ...
    IEnumerator IncreaseLightningReload() {
        float dur = 0;
        while (LightningCast && dur < TotemIncreasedTime && TotemNotChanged) {
            CurLightningAttackSpeed = TotemIncreasedAA;
            yield return null;
        }
        CurLightningAttackSpeed = LightningAAReload;
        ChLightning = false;
    ...
    private GameObject HealingPlacePrefab;
    IEnumerator CreateHealingPlace() {
        ...
        while (!LightningCast && dur < HealingPlaceCreatedTime && TotemNotChanged) {
            Dictionary<int, bool> enemies = new Dictionary<int, bool>();
            bool inside = false;
    ...
    void CastThunder() {
        ...
        if (sign < 0.5)
            RandomZ = -RandomZ;
        audioPlayer.PlayOneShot(RandomThunderClip);
    ...
    IEnumerator ChainLightning(GameObject Ene) {
        StartCoroutine(LightningAA(Ene, 1));
        int ChainedTime = 0;
        GameObject Last = Ene;
        int LastHash = Ene.transform.GetHashCode();
        Dictionary<int, int> Chained = new Dictionary<int, int>();
        Chained.Add(LastHash, 1);
        yield return new WaitForSeconds(ChainLightningCharge);
        while (ChainedTime < ChainLightningTime && LightningCast) {
            float ShortestDis = LightningAARange;
    ...
    IEnumerator LightningInstantiateFromA(GameObject A, GameObject B) {
        ...
        for (var i = 1; i < LightningCurvePts+1; i++)
            v2[i] = UnityEngine.Random.Range(0, 360);
        GameObject LightningPrefab = Instantiate(Lightning2, rightHand.position, Quaternion.identity);
        LineRenderer LR = LightningPrefab.GetComponent<LineRenderer>();
        int glowtimes = 0;
        int StopGlow = 5;
        while (glowtimes < LightningGlowingTimes) {
